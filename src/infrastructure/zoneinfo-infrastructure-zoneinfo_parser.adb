...
with Interfaces;
with Functional.Try;
with ZoneInfo.Domain.Error;
with ZoneInfo.Domain.Value_Object.StarterLib_Header;
with ZoneInfo.Domain.Value_Object.Transition;
with ZoneInfo.Domain.Value_Object.Timezone_Type;
with ZoneInfo.Domain.Value_Object.Epoch_Seconds;
with ZoneInfo.Domain.Value_Object.UTC_Offset;

package body ZoneInfo.Infrastructure.StarterLib_Parser is

   use Ada.Streams;
   use Ada.Streams.Stream_IO;
   use Interfaces;
   use ZoneInfo.Domain.Error;
   use ZoneInfo.Domain.Error.Error_Strings;
   use ZoneInfo.Domain.Value_Object.StarterLib_Header;
   use ZoneInfo.Domain.Value_Object.Transition;
   use ZoneInfo.Domain.Value_Object.Timezone_Type;
   ...
   function To_Integer_32 is new
     Ada.Unchecked_Conversion (Source => Unsigned_32, Target => Integer_32);

   function To_Integer_64 is new
     Ada.Unchecked_Conversion (Source => Unsigned_64, Target => Integer_64);

   --  ========================================================================
   --  Domain Result Types
   --  ========================================================================

   package Int32_Result is new
     Domain.Error.Result.Generic_Result (T => Integer_32);

   package Int64_Result is new
     Domain.Error.Result.Generic_Result (T => Integer_64);

   package Byte_Result is new
     Domain.Error.Result.Generic_Result (T => Stream_Element);

   package String_Result is new
     Domain.Error.Result.Generic_Result (T => Unbounded_String);
      ...
      if Exc_Name = "STORAGE_ERROR" then
         return
           (Kind    => Resource_Error,
            Message =>
              To_Bounded_String
                ("Out of memory during stream I/O: " & Exception_Message (Occ)));
      ...
      begin
         ...
         if Last /= 4 then
            raise Constraint_Error with
              "Insufficient data for Int32 (read" & Last'Image
              & " bytes, expected 4)";
         end if;

         Unsigned_Val :=
           Shift_Left (Unsigned_32 (Bytes (1)), 24)
           or Shift_Left (Unsigned_32 (Bytes (2)), 16)
           or Shift_Left (Unsigned_32 (Bytes (3)), 8)
           or Unsigned_32 (Bytes (4));

         return To_Integer_32 (Unsigned_Val);
      end Raw_Read_Int32;

      function Try_Read is new
        Functional.Try.Try_To_Result
          (T             => Integer_32,
           E             => Error_Type,
   ...
   is
      ...
      begin
         ...
         if Last /= 8 then
            raise Constraint_Error with
              "Insufficient data for Int64 (read" & Last'Image
              & " bytes, expected 8)";
         end if;

         Unsigned_Val :=
           Shift_Left (Unsigned_64 (Bytes (1)), 56)
           or Shift_Left (Unsigned_64 (Bytes (2)), 48)
           or Shift_Left (Unsigned_64 (Bytes (3)), 40)
           or Shift_Left (Unsigned_64 (Bytes (4)), 32)
           or Shift_Left (Unsigned_64 (Bytes (5)), 24)
           or Shift_Left (Unsigned_64 (Bytes (6)), 16)
           or Shift_Left (Unsigned_64 (Bytes (7)), 8)
           or Unsigned_64 (Bytes (8));
      ...
      function Try_Read is new
        ...
        (T             => Integer_64,
         E             => Error_Type,
         Result_Type   => Int64_Result.Result,
         Ok            => Int64_Result.Ok,
         Err           => Int64_Result.From_Error,
         Map_Exception => From_Exception,
         Action        => Raw_Read_Int64);
   begin
      return Try_Read;
   end Read_Int64;
   ...
   is
      ...
      begin
         ...
         if Last /= 1 then
            raise Constraint_Error with
              "Insufficient data for byte (read" & Last'Image
              & " bytes, expected 1)";
         end if;
         return Byte (1);
      end Raw_Read_Byte;
      ...
      function Try_Read is new
        ...
        (T             => Stream_Element,
         E             => Error_Type,
         Result_Type   => Byte_Result.Result,
         Ok            => Byte_Result.Ok,
         Err           => Byte_Result.From_Error,
         Map_Exception => From_Exception,
   ...
   is
      ...
      begin
         ...
         if Last /= Stream_Element_Offset (Length) then
            raise Constraint_Error with
              "Insufficient data for string (read" & Last'Image
      ...
      function Try_Read is new
        ...
        (T             => Unbounded_String,
         E             => Error_Type,
         Result_Type   => String_Result.Result,
         Ok            => String_Result.Ok,
         Err           => String_Result.From_Error,
         Map_Exception => From_Exception,
   ...
   is
      function Raw_Parse_Header return StarterLib_Data_Type is
         Magic : Stream_Element_Array (1 .. 4);
         Last  : Stream_Element_Offset;
         Data  : StarterLib_Data_Type;
      begin
         --  Read magic number
         Read (Stream.all, Magic, Last);
         if Last /= 4
           or else (Magic (1) /= Character'Pos ('T')
                    or else Magic (2) /= Character'Pos ('Z')
                    or else Magic (3) /= Character'Pos ('i')
                    or else Magic (4) /= Character'Pos ('f'))
         then
            raise Constraint_Error with "Invalid magic number (not 'ZoneInfo')";
         end if;
         ...
         begin
            ...
            declare
               ...
               Std_Wall_Res   : constant Int32_Result.Result :=
                 Read_Int32 (Stream);
               Leap_Res       : constant Int32_Result.Result :=
                 Read_Int32 (Stream);
               Transition_Res : constant Int32_Result.Result :=
                 Read_Int32 (Stream);
               Type_Res       : constant Int32_Result.Result :=
                 Read_Int32 (Stream);
            ...
            begin
               ...
               begin
                  case Version is
                     ...
                     when others =>
                        raise Constraint_Error with
                          "Invalid or unsupported ZoneInfo version:"
                          & Version'Image;
                  end case;
               end;
      ...
      function Try_Parse is new
        Functional.Try.Try_To_Result
        (T             => StarterLib_Data_Type,
         E             => Error_Type,
         Result_Type   => Parse_Result.Result,
         Ok            => Parse_Result.Ok,
         Err           => Parse_Result.From_Error,
         Map_Exception => From_Exception,
   ...
   procedure Parse_Data_Section
     (Stream    : not null Stream_Access;
      ...
      Data      : in out ZoneInfo.Domain.StarterLib_Data.StarterLib_Data_Type;
      Use_64bit : Boolean) is
   begin
            ...
            if Use_64bit then
               ...
               begin
                  if Int64_Result.Is_Error (Time_R) then
                     raise Constraint_Error with
                       To_String (Int64_Result.Error_Info (Time_R).Message);
                  end if;
               ...
               begin
                  if Int32_Result.Is_Error (Time_R) then
                     raise Constraint_Error with
                       To_String (Int32_Result.Error_Info (Time_R).Message);
                  end if;
      ...
      for I in 1 .. Header.Transition_Count loop
         ...
         begin
            if Byte_Result.Is_Error (Type_Index_R) then
               raise Constraint_Error with
                 To_String (Byte_Result.Error_Info (Type_Index_R).Message);
            end if;
      ...
      begin
         for I in 1 .. Header.Type_Count loop
            declare
               UTC_Off_R : constant Int32_Result.Result :=
                 Read_Int32 (Stream);
            begin
               if Int32_Result.Is_Error (UTC_Off_R) then
                  raise Constraint_Error with
                    To_String (Int32_Result.Error_Info (UTC_Off_R).Message);
               end if;
            ...
            begin
               if Byte_Result.Is_Error (DST_R) then
                  raise Constraint_Error with
                    To_String (Byte_Result.Error_Info (DST_R).Message);
               end if;
            ...
            begin
               if Byte_Result.Is_Error (Abbr_Idx_R) then
                  raise Constraint_Error with
                    To_String (Byte_Result.Error_Info (Abbr_Idx_R).Message);
               end if;
   ...
   begin
      declare
         V1_Header     : StarterLib_Header_Type;
         ...
         if Is_Version_2_Or_Later (V1_Header) then
            declare
               V1_Size : constant Natural :=
                 V1_Header.Transition_Count * 5
                 + V1_Header.Type_Count * 6
                 + V1_Header.Abbrev_Chars
                 + V1_Header.Leap_Count * 8
                 + V1_Header.Standard_Wall_Count
                 + V1_Header.UTC_Local_Count;
            begin
   ...
   function Parse_From_File (File_Path : String) return Parse_Result_Type is
      function Raw_Parse_File return StarterLib_Data_Type is
         File   : File_Type;
      ...
      begin
         ...
         if Parse_Result.Is_Error (Result) then
            raise Constraint_Error with
              To_String (Parse_Result.Error_Info (Result).Message);
         end if;
